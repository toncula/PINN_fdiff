...
hidden_size = 32
learning_rate = 0.001
save_dir = 'pinn_change_new_saves LA92'
...
save_dir = 'pinn_change_new_saves 10deg LA92'
save_dir = os.path.join(save_dir)
setup_seed(seed)
if not os.path.exists(save_dir):
     os.mkdir(save_dir)
...
validation_dataset = MatDNNDataset("data/Panasonic 18650PF Data/10degC/Drive Cycles/03-27-17_09.06 10degC_HWFET_Pan18650PF.mat")
train_loaders = list()
for d in train_datasets:
    temp = DataLoader(dataset=d, batch_size=batch_size, shuffle=True)
    train_loaders.append(temp)
...
save_dir = 'pinn_change_new_saves -20deg LA92'
save_dir = os.path.join(save_dir)
setup_seed(seed)
if not os.path.exists(save_dir):
     os.mkdir(save_dir)
...
validation_dataset = MatDNNDataset("data/Panasonic 18650PF Data/-20degC/Drive cycles/06-23-17_23.35 n20degC_HWFET_Pan18650PF.mat")
train_loaders = list()
for d in train_datasets:
    temp = DataLoader(dataset=d, batch_size=batch_size, shuffle=True)
    train_loaders.append(temp)
