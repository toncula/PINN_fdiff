...
model_mlp = RNN(input_size,hidden_size,num_layers,1,gaussian_noise=0.0)

"""0deg"""
save_dir = "pinn_fdiff/picture"
save_dir = os.path.join(save_dir)
model = torch.load(
    "pinn_fdiff/pinn_rnn_saves -20degC/model_pinn.pt", weights_only=False
).to(device)
model_mlp = torch.load(
    "pinn_fdiff/pinn_rnn_saves -20degC/model_rnn.pt", weights_only=False
).to(device)
...
plt.savefig(
    os.path.join(save_dir, "soc_pred_n20 HWEFT_rnn.jpg"),
    bbox_inches="tight",
    pad_inches=0,
)
...
plt.savefig(
    os.path.join(save_dir, "soc_error_n20 HWEFT_rnn.jpg"),
    bbox_inches="tight",
    pad_inches=0,
)

np.save("pinn_fdiff/picture/HWEFT_SOC_-20.npy", soc)
np.save("pinn_fdiff/picture/PINN_RNN_HWEFT_SOC_-20.npy", y_fsmm)
np.save("pinn_fdiff/picture/RNN_HWEFT_SOC_-20.npy", y_mlp)
np.save("pinn_fdiff/picture/LOSS_PINN_RNN_HWEFT_SOC_-20.npy", loss_fsmm)
np.save("pinn_fdiff/picture/LOSS_RNN_HWEFT_SOC_-20.npy", loss_mlp)
