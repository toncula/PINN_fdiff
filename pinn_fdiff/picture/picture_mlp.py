...
model = torch.load(
    "pinn_fdiff/pinn_change_new_saves -10deg US06/model_pinn.pt", weights_only=False
).to(device)
model_mlp = torch.load(
    "pinn_fdiff/pinn_change_new_saves -10deg US06/model_mlp.pt", weights_only=False
).to(device)
...
for i, (x, y) in enumerate(validation_dataset):
    ...
    # 记录真实值
    soc.extend(y.detach().cpu().numpy().flatten().tolist())
    a.extend(x[:, -1].detach().cpu().numpy().flatten().tolist())

    # FSMML预测
    y_pre, _, _ = model(x)
    y_fsmm.extend(y_pre.detach().cpu().numpy().flatten().tolist())

    # MLP预测
    y_pre_mlp = model_mlp(x)
    y_mlp.extend(y_pre_mlp.detach().cpu().numpy().flatten().tolist())


# 计算 MAE / MSE
loss_fsmm = np.abs(np.array(y_fsmm) - np.array(soc))
loss_mlp = np.abs(np.array(y_mlp) - np.array(soc))
mae_fsmm = np.mean(loss_fsmm)
mae_mlp = np.mean(loss_mlp)
# MSE：平方误差平均值
mse_fsmm = np.mean(loss_fsmm**2)
mse_mlp = np.mean(loss_mlp**2)


print(f"[MLP+PINN] MAE: {mae_fsmm:.6f}, MSE: {mse_fsmm:.6f}")
print(f"[MLP Only ] MAE: {mae_mlp:.6f}, MSE: {mse_mlp:.6f}")
fig,axes = plt.subplots(1,1,figsize=(28,20))
axes.plot(a,soc,color='b',label='Acture SOC')
axes.plot(a, y_fsmm, color="g", label="MLP+PINN")
axes.plot(a,y_mlp,color='r',label='MLP')
axes.legend()
plt.ylabel('SOC')
plt.xlabel('Time (s) T=-10°C')
plt.savefig(
    os.path.join(save_dir, "soc_pred_-10deg US06_mlp.jpg"),
    bbox_inches="tight",
    pad_inches=0,
)
...
fig,axes = plt.subplots(1,1,figsize=(28,20))
axes.plot(a, loss_fsmm, color="g", label="MLP+PINN")
axes.plot(a,loss_mlp,color='r',label='MLP')
axes.legend()
plt.ylabel('SOC error')
plt.xlabel('Time (s) T=-20°C')

plt.savefig(
    os.path.join(save_dir, "soc_error_-10deg US06_mlp.jpg"),
    bbox_inches="tight",
    pad_inches=0,
)
...
np.save("pinn_fdiff/picture/PINN_MLP_US06_SOC_-10.npy", y_fsmm)
np.save("pinn_fdiff/picture/MLP_US06_SOC_-10.npy", y_mlp)
np.save("pinn_fdiff/picture/LOSS_PINN_MLP_US06_SOC_-10.npy", loss_fsmm)
np.save("pinn_fdiff/picture/LOSS_MLP_US06_SOC_-10.npy", loss_mlp)
