...
model = torch.load(
    "pinn_fdiff/pinn_rnn_saves -10degC/model_pinn.pt", weights_only=False
).to(device)
model_mlp = torch.load(
    "pinn_fdiff/pinn_rnn_saves -10degC/model_rnn.pt", weights_only=False
).to(device)

validation_dataset = MatDNNDataset(
    "data/Panasonic 18650PF Data/-10degC/Drive Cycles/06-07-17_08.39 n10degC_HWFET_Pan18650PF.mat",
    sequence_length,
)
...
plt.savefig(
    os.path.join(save_dir, "soc_pred_-10deg HWEFt_rnn.jpg"),
    bbox_inches="tight",
    pad_inches=0,
)
...
plt.savefig(
    os.path.join(save_dir, "soc_error_-10deg HWEFT_rnn.jpg"),
    bbox_inches="tight",
    pad_inches=0,
)

np.save("pinn_fdiff/picture/HWEFT_SOC_-10.npy", soc)
np.save("pinn_fdiff/picture/PINN_RNN_HWEFT_SOC_-10.npy", y_fsmm)
np.save("pinn_fdiff/picture/RNN_HWEFT_SOC_-10.npy", y_mlp)
np.save("pinn_fdiff/picture/LOSS_PINN_RNN_HWEFT_SOC_-10.npy", loss_fsmm)
np.save("pinn_fdiff/picture/LOSS_RNN_HWEFT_SOC_-10.npy", loss_mlp)
