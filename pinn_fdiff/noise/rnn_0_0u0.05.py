...
class MatDNNDataset(Dataset):
    def __init__(self, root , sequence_length,noise=0.0):
...
hidden_size = 32
learning_rate = 0.001
save_dir = 'pinn_fdiff/pinn_rnn_saves 0degC u0.05'
...
validation_dataset = MatDNNDataset("data/Panasonic 18650PF Data/0degC/Drive cycles/06-02-17_10.43 0degC_HWFET_Pan18650PF.mat",sequence_length,0.05)
train_loaders = list()
...
for i,(x,y) in enumerate(validation_loader):
    ...
    pinn_mae_loss.append(loss_t(pinn_pre_y, y).item())
    mlp_mae_loss.append(loss_t(mlp_y, y).item())
    x = x.to('cpu')
    y = y.to('cpu')
...
pinn_mae_total_loss = sum(pinn_mae_loss)/t
total_mae_loss = sum(mlp_mae_loss)/t
print('pinn_loss:',total_pinn_loss,"rnn_loss:",total_loss)
print('pinn_mae_loss:',pinn_mae_total_loss,"rnn_mae_loss:",total_mae_loss)
...
with open(os.path.join(save_dir, 'result.pkl'), 'wb') as f:
    pkl.dump({'epochs': epochs,
              ...
              'pinn_loss':pinn_loss,
              'rnn_loss':mlp_loss,
              'pinn_mae_loss': pinn_mae_loss,
              'rnn_mae_loss': mlp_mae_loss}, f)
