    ...
    def _calculate_Up_sequence(self, out):
        ...
        for t in range(seq_len):
            ...
            padded_voltage = torch.cat(
                [torch.zeros((batch_size, pad_size), device=device), voltage_window], dim=1
            ) 

            time_window = time[:, start_idx : t + 1]
            valid_window = t + 1 - start_idx
            if valid_window > 1:
                time_diff = (time_window[:, -1] - time_window[:, 0]) / (valid_window - 1)
            else:
...
hidden_size = 32
learning_rate = 0.001
save_dir = 'pinn_fdiff/pinn_lstm_saves -10 LA92'
...
validation_dataset = MatDNNDataset(
    "data/Panasonic 18650PF Data/-10degC/Drive Cycles/06-07-17_08.39 n10degC_LA92_Pan18650PF.mat",
    sequence_length,
)
train_loaders = list()
