...
seed = 7084#random.randint(1,10000)
print(seed)
batch_size = 256
num_epochs = 100
input_size = 4
hidden_size = 32
learning_rate = 0.001
save_dir = 'pinn_fdiff/pinn_change_new_saves US06'
...
for i, (x, y) in enumerate(validation_loader):
    ...
    p_loss.append(loss_t(pinn_pre_y, y).item())
    m_loss.append(loss_t(mlp_y, y).item())

test = range(t)
total_pinn_loss = sum(pinn_loss) / t
total_loss = sum(mlp_loss) / t
t_pinn_loss = sum(p_loss) / t
t_loss = sum(m_loss) / t
print('pinn_loss:', total_pinn_loss, "mlp_loss:", total_loss)
print('pinn_loss:', t_pinn_loss, "mlp_loss:", t_loss)
