...
class LSTM(nn.Module):
    def __init__(
        self, input_size, hidden_size, num_layers, num_classes, gaussian_noise
    ):
...
seed = 4479  # random.randint(1,10000)
print(seed)
sequence_length = 20
batch_size = 256
num_epochs = 50
...
hidden_size = 32
learning_rate = 0.001
save_dir = "pinn_fdiff/3_layers_lstm/pinn_lstm_saves -10 UDDS"
...
validation_dataset = MatDNNDataset(
    "data/Panasonic 18650PF Data/-10degC/Drive Cycles/06-07-17_08.39 n10degC_UDDS_Pan18650PF.mat",
    sequence_length,
)
train_loaders = list()
    ...
    for seti, loader in enumerate(train_loaders):
        ...
        for i, (x, y) in enumerate(loader):
            ...
            with torch.backends.cudnn.flags(enabled=False):
                ...
                loss = (
                    criterion(soc, y)
                    + 0.5 * torch.mean(torch.square(loss1))
                    + 0.5 * torch.mean(torch.square(loss2))
                )
...
for i, b in enumerate(c):
    ...
    axes.plot(
        epochs[b:], pinn_soc_loss[b:], color="b", linewidth=2, label="pinn_soc_loss"
    )
    ...
    plt.savefig(
        os.path.join(save_dir, "epochs{}-40.pdf".format(b + 1)),
        bbox_inches="tight",
        pad_inches=0,
    )
