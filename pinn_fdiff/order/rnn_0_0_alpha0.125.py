...
hidden_size = 32
learning_rate = 0.001
save_dir = 'pinn_fdiff/pinn_rnn_saves 0degC alpha0.125'
...
model = Fsmm(input_size,hidden_size,num_layers,alpha = 0.125).to(device)
    ...
    for seti, loader in enumerate(train_loaders):
        ...
        for i, (x, y) in enumerate(loader):
            ...
            with torch.backends.cudnn.flags(enabled=False):
                ...
                loss = (
                    criterion(soc, y)
                    + 0.25 * torch.mean(torch.square(loss1))
                    + 0.25 * torch.mean(torch.square(loss2))
                )
