...
epochs = list()
pinn_soc_loss = list()
lstm_epoch_loss = list()
min_loss = 999
best_model = Fsmm(input_size,hidden_size,num_layers).to(device)
optimizer_mlp = torch.optim.Adam(model_lstm.parameters(),lr=learning_rate)
for epoch in range(num_epochs):
        ...
        for i,(x,y) in enumerate(loader):
            with torch.backends.cudnn.flags(enabled=False):
               ...
               if epoch < 3 and epoch >0:
                   pinn_single_loss.append(loss_need)
    ...
    for loader in train_loaders:
        for i,(x,y) in enumerate(loader):
            ...
            optimizer_mlp.step()
            if epoch < 3 and epoch > 0:
                lstm_single_loss.append(loss_lstm)
            total_train_loss_mlp += loss_lstm.item()
            num_train_batches += 1
    averge_loss_mlp = total_train_loss_mlp / num_train_batches
    lstm_epoch_loss.append(averge_loss_mlp)
    print("epoch:",epoch+1,
          "loss:",averge_loss_mlp,
          )
    if epoch == 2:
        fig,ax = plt.subplots(1,1,figsize=(20,14))
        ax.plot(range(len(lstm_single_loss)),pinn_single_loss,color='b',linewidth=0.3, label='pinn_soc_loss')
        ax.plot(range(len(lstm_single_loss)), lstm_single_loss, color='g', linewidth=0.3, label='lstm_soc_loss')
        ax.legend()
        plt.ylabel('loss')
        plt.savefig(os.path.join(save_dir, 'train.pdf'), bbox_inches='tight', pad_inches=0)
...
for i,b in enumerate(c):
    ...
    axes.plot(epochs[b:],lstm_epoch_loss[b:],color='g',linewidth=2,label='lstm')
    axes.legend()
    plt.xlabel('epochs')
    plt.ylabel('loss')
...
for i,(x,y) in enumerate(validation_loader):
    ...
    lstm_loss.append(loss(mlp_y,y).item())
    x = x.to('cpu')
    y = y.to('cpu')

test = range(t)
total_pinn_loss = sum(pinn_loss)
total_loss = sum(lstm_loss)
print('pinn_loss:',total_pinn_loss,"lstm_loss:",total_loss)
fig,axes = plt.subplots(1,1,figsize=(20,14))
axes.plot(test,pinn_loss,color='b',label='pinn_soc_loss')
axes.plot(test,lstm_loss,color='g',label='lstm')
axes.legend()
plt.xlabel('epochs')
plt.ylabel('loss')
...
with open(os.path.join(save_dir, 'result.pkl'), 'wb') as f:
    pkl.dump({'epochs': epochs,
              ...
              'mlp_train_loss': lstm_epoch_loss,
              'pinn_loss':pinn_loss,
              'mlp_loss':lstm_loss}, f)
