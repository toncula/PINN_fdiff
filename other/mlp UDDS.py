...
model = torch.load('../pinn_change_new_saves LA92/model_pinn.pt').to(device)
model_mlp = torch.load('../pinn_change_new_saves LA92/model_mlp.pt').to(device)
validation_dataset = MatDNNDataset("../data/Panasonic 18650PF Data/0degC/Drive cycles/06-02-17_17.14 0degC_UDDS_Pan18650PF.mat")
...
model = torch.load('../pinn_change_new_saves 10deg LA92/model_pinn.pt')
model_mlp = torch.load('../pinn_change_new_saves 10deg LA92/model_mlp.pt')
validation_dataset = MatDNNDataset("../data/Panasonic 18650PF Data/10degC/Drive Cycles/03-27-17_09.06 10degC_UDDS_Pan18650PF.mat")
validation_loader = DataLoader(validation_dataset, batch_size=batch_size, shuffle=True)
loss = nn.MSELoss()
pinn_loss = 0
mlp_loss = 0
...
model = torch.load('../pinn_change_new_saves -10deg LA92/model_pinn.pt').to(device)
model_mlp = torch.load('../pinn_change_new_saves -10deg LA92/model_mlp.pt').to(device)

validation_dataset = MatDNNDataset("../data/Panasonic 18650PF Data/-10degC/Drive Cycles/06-07-17_08.39 n10degC_UDDS_Pan18650PF.mat")
validation_loader = DataLoader(validation_dataset, batch_size=batch_size, shuffle=True)
loss = nn.MSELoss()
pinn_loss = 0
mlp_loss = 0
...
model = torch.load('../pinn_change_new_saves -20deg LA92/model_pinn.pt').to(device)
model_mlp = torch.load('../pinn_change_new_saves -20deg LA92/model_mlp.pt').to(device)
validation_dataset = MatDNNDataset("../data/Panasonic 18650PF Data/-20degC/Drive cycles/06-23-17_23.35 n20degC_UDDS_Pan18650PF.mat")
validation_loader = DataLoader(validation_dataset, batch_size=batch_size, shuffle=True)
loss = nn.MSELoss()
pinn_loss = 0
mlp_loss = 0
