...
model = torch.load('../pinn_rnn_saves 0degC/model_pinn.pt')
model_mlp = torch.load('../pinn_rnn_saves 0degC/model_rnn.pt')
validation_dataset = MatDNNDataset("../data/Panasonic 18650PF Data/0degC/Drive cycles/06-01-17_10.36 0degC_LA92_Pan18650PF.mat",sequence_length)
...
model = torch.load('../pinn_rnn_saves 10degC/model_pinn.pt')
model_mlp = torch.load('../pinn_rnn_saves 10degC/model_rnn.pt')
validation_dataset = MatDNNDataset("../data/Panasonic 18650PF Data/10degC/Drive Cycles/03-27-17_09.06 10degC_LA92_Pan18650PF.mat",sequence_length)
validation_loader = DataLoader(validation_dataset, batch_size=batch_size, shuffle=True)
loss = nn.MSELoss()
pinn_loss = 0
mlp_loss = 0
...
model = torch.load('../pinn_rnn_saves -10degC/model_pinn.pt').to(device)
model_mlp = torch.load('../pinn_rnn_saves -10degC/model_rnn.pt').to(device)

validation_dataset = MatDNNDataset("../data/Panasonic 18650PF Data/-10degC/Drive Cycles/06-07-17_08.39 n10degC_LA92_Pan18650PF.mat",sequence_length)
validation_loader = DataLoader(validation_dataset, batch_size=batch_size, shuffle=True)
loss = nn.MSELoss()
pinn_loss = 0
mlp_loss = 0
...
model = torch.load('../pinn_rnn_saves -20degC/model_pinn.pt').to(device)
model_mlp = torch.load('../pinn_rnn_saves -20degC/model_rnn.pt').to(device)
validation_dataset = MatDNNDataset("../data/Panasonic 18650PF Data/-20degC/Drive cycles/06-23-17_23.35 n20degC_LA92_Pan18650PF.mat",sequence_length)
validation_loader = DataLoader(validation_dataset, batch_size=batch_size, shuffle=True)
loss = nn.MSELoss()
pinn_loss = 0
mlp_loss = 0
