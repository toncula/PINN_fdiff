...
class MatDNNDataset(Dataset):
    def __init__(self, root , sequence_length,noise=0.0):
        ...
        self.X = torch.cat((self.V,self.I,self.T,self.time),1) + torch.randn(self.X.size()) * self.stddev
...
model = torch.load('pinn_rnn_saves/model_pinn.pt').to(device)
model_mlp = RNN(input_size,hidden_size,num_layers,1,gaussian_noise=0.0)
model_mlp = torch.load('pinn_rnn_saves/model_rnn.pt').to(device)


validation_dataset = MatDNNDataset("data/Panasonic 18650PF Data/0degC/Drive cycles/06-02-17_10.43 0degC_HWFET_Pan18650PF.mat",sequence_length,0.02)
